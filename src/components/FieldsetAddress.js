...
class FieldsetAddress extends React.Component {
  ...
  formatPostcode = postcode => {
    ...
    return (
      noSpaces.substring(0, noSpaces.length - 3) +
      ' ' +
      noSpaces.substring(noSpaces.length - 3)
    )
  }
  ...
  getPossibleAddresses = async () => {
    ...
    const postcodeForQuery = this.state.postcode
      .toLowerCase()
      .replace(/\s/g, '')
    ...
    try {
      ...
      this.setState({
        addressSelectorOptions: optionsArray.map(option =>
  ...
  componentDidMount() {
    ...
    postcode.addEventListener(
      'invalid',
      () => {
        const failureStates = ['valueMissing', 'patternMismatch']

        for (let i = 0; i < failureStates.length; i++) {
          const failureState = failureStates[i]
          if (
            postcode.validity[failureState] &&
            postcodeValidationMessages[failureState]
          ) {
            this.setState({
    ...
    addressSelector.addEventListener(
      'invalid',
      () => {
      ...
      if (addressSelector.value === '') {
        ...
        this.setState(
          ...
          () => {
            ...
            document
              .getElementById(inputId + '-line-1')
              .dispatchEvent(new Event('change'))
            document
              .getElementById(inputId + '-line-2')
              .dispatchEvent(new Event('change'))
            document
              .getElementById(inputId + '-line-3')
              .dispatchEvent(new Event('change'))
            document
              .getElementById(inputId + '-town')
              .dispatchEvent(new Event('change'))
            document
              .getElementById(inputId + '-county')
              .dispatchEvent(new Event('change'))
          }
        )
        return
        ...
        this.setState(
          {
            ...
            addressLine2:
              address.line2 !== '' ? address.line2 : address.locality,
            addressLine3:
              address.line2 !== '' && address.locality !== ''
                ? address.locality
                : address.line3,
            town: address.town,
    ...
    return (
      <div>
        <fieldset className={fieldsetClassNames.join(' ')}>
          ...
          <div
            className={
              'field ' + (this.state.postcodeValidationMessage && ' has-error')
            }
          >
            <label className={'label'} htmlFor={inputId + '-postcode'}>
              Find your address using your postcode
            </label>
            ...
            {this.state.addressApiError && (
              <p className="hint">
                Sorry! Our address lookup isn't working at the minute - please
                fill in the fields below.
              </p>
            )}
            <div className={addressSelectorClasses.join(' ')}>
              <label className="label" htmlFor={inputId + '-address-selector'}>
                Select your address
              </label>
              ...
              <div className="control">
                <div className="select">
                  ...
                  >
                    <option value="" key={'address-option-default'}>
                      (Select your address)
                    </option>
