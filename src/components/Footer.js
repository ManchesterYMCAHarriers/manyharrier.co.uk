...
const FooterMenuLink = ({title, to}) => (
  <Link className="text-center md:text-left w-full block flex-shrink-0 flex-grow-0 p-2 border-b-2 border-gray-800 hover:border-white focus:border-white bg-transparent hover:bg-gray-800 focus:bg-gray-800 focus:outline-none" to={to}>
    {title}
  </Link>
)

const FooterSocialLink = ({title, to, children}) => (
  <a title={title} href={to}
...
const Footer = class extends React.Component {
  render() {
    ...
    `
    } render={({site: { siteMetadata: { title, social: {facebook, instagram, strava, twitter} } } }) => (
      <footer className="bg-black text-white mt-8">
        <div className="max-w-6xl mx-auto flex flex-wrap">
          <div className="w-full md:w-auto mt-8 md:ml-4 flex-shrink-0 flex-grow flex flex-wrap md:flex-no-wrap md:flex-col items-center md:items-start justify-center md:justify-between">
            <FooterLogo />
            <div
              className="w-full max-w-2xl mt-8 flex-shrink-0 flex-grow-0 flex items-center md:items-end justify-around md:justify-start">
          ...
          <div className="w-full my-8 mx-2 flex-shrink-0 flex-grow-0 text-center">
            <span className="whitespace-no-wrap">Copyright &copy; {new Date().getUTCFullYear()}</span>{' '}
